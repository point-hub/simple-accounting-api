...
import { RetrieveManyRepository as ChartOfAccountRetrieveManyRepository } from '@/modules/chart-of-accounts/repositories/retrieve-many.repository';

import { OpeningBalanceRepository } from '../repositories/opening-balance.repository';
...
export const retrieveManyController: IController = async (controllerInput: IControllerInput) => {
  ...
  try {
    ...
    const chartOfAccountRetrieveManyRepository = new ChartOfAccountRetrieveManyRepository(controllerInput.dbConnection);
    const retrieveManyRepository = new RetrieveManyRepository(controllerInput.dbConnection);
    const openingBalanceRepository = new OpeningBalanceRepository(controllerInput.dbConnection);

    // Initialize use case with dependencies
    const retrieveManyUseCase = new RetrieveManyUseCase({
      chartOfAccountRetrieveManyRepository,
